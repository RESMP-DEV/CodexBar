# Review guidance for RESMP-DEV/CodexBar
...
- Review RESMP-DEV/CodexBar as the source of truth for Show usage stats for OpenAI Codex and Claude Code, without having to login.
...
- Preserve lifecycle-safe state, offline and retry behavior, user data integrity, permission boundaries, and compatibility with supported device and OS versions.
- Review camera, file, network, notification, and background-work paths for cancellation, duplicate work, resource cleanup, privacy, and actionable user-visible failure states.
- UI changes must retain keyboard or switch access where applicable, screen-reader labels, scalable text, focus order, contrast, and loading, empty, error, and narrow-screen behavior.
...
- For Swift changes, review actor isolation, Sendable assumptions, async cancellation, lifecycle and retain cycles, platform availability, sandbox/entitlement changes, and stable public interfaces.
- Require focused tests on the supported Apple platform; previews and compilation do not prove lifecycle or concurrency behavior.
